...
**Phase 2.6 — Snapshot Networking: Implementation (Шаг 2.6.4: сквозная связка репликации с транспортом и tick-loop).**

Шаг 2.6.3 Client Replication Receiver & FSM — **реализован**: 558/558 EditMode тестов green (429 baseline + 129 новых клиентских тестов репликации), Zero-GC hot path валидирован. Изменения в working tree, commit ещё не выполнен. `LocalMatchHost`, клиентский рендер и транспортный pump не изменялись — сквозная связка это Шаг 2.6.4.

## Last Commit

`6c5e37e` — `feat(server): implement replication diff engine and history ring (Phase 2.6 step 2)` (предыдущие: `028e646` — Шаг 2.6.1 Core delta wire codec, `dadc593` — `feat: implement network transport (Phase 2.5)`)
...
- Phase 2.6 Step 2.6.3 Client Replication Receiver & FSM — реализован, commit ожидается (558/558 EditMode passed; 129 новых тестов; Zero-GC hot path клиента валидирован)
...
- Зафиксированные отклонения/уточнения Шага 2.6.3 (требуют отражения в следующей ревизии R&D, архитектуру не меняют):
  - `SnapshotAck` = **34 байта**, а не «33 байта» из текста R&D §4.2: offsets 0/1/2/10/18/26 в сумме дают 34, нормативны offsets. Uplink-бюджет — 340 Б/с при 10 Hz, а не 330 Б/с.
  ...
  - Возраст keyframe-базы (`Now − BaseKeyframeTick > 120`) не используется как триггер REBASING: при плановом интервале keyframe 15–20 с (OD-11) это давало бы ложный ребейз каждые 6 с. Реализован нормативный Apply-Guard ADR-010: REBASING при `BaseTick − LastAppliedTick > 120` или несовпадении `KeyframeRef`.
