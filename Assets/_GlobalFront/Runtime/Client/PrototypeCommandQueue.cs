...
namespace GlobalFront.Client
{
    ...
    /// deterministic ordering and application to local units. The queue is
    /// designed so that a future network layer can intercept
    /// <see cref="QueueMove"/> / <see cref="QueueAttack"/> to send commands to
    /// an authoritative server, and <see cref="ApplyPending"/> can be replaced
    /// by snapshot-driven reconciliation without changing the controller.
    /// </summary>
    public sealed class PrototypeCommandQueue
    {
        /// <summary>
        /// Immutable wrapper around either a <see cref="MoveCommand"/> or an
        /// <see cref="AttackCommand"/>. Public so that future serializers and
        /// tests can enumerate pending commands without reflection.
        /// </summary>
